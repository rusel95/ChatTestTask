...
final class TestChatItemsDecorator: ChatItemsDecoratorProtocol {
            ...
            if let currentMessage = chatItem as? MessageModelProtocol {
                ...
                if self.showsStatusForMessage(currentMessage) {
                    additionalItems.append(
                        DecoratedChatItem(
                            chatItem: SendingStatusModel(uid: "\(currentMessage.uid)-decoration-status",
                ...
                isShowingSelectionIndicator = self.messagesSelector.isActive &&
                                              self.messagesSelector.canSelectMessage(currentMessage)
            }
    ...
    private func separationAfterItem(_ current: ChatItemProtocol?, next: ChatItemProtocol?) -> CGFloat {
        ...
        } else if nextMessage.date.timeIntervalSince(currentMessage.date) >
